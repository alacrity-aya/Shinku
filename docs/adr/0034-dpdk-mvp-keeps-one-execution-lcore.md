...
The process affinity policy again selects only the lowest usable CPU from `sched_getaffinity()` and maps MAIN lcore ID
zero to it. Cache Hit responses in 9B can use the client-side TX queue directly because the same lcore owns all packet
I/O.
