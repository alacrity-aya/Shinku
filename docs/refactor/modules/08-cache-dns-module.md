...
- Log-ring `EINTR` completes the quantum successfully; other log errors are non-fatal warnings.
- Packet-ring `EINTR` completes the quantum successfully; other packet errors return `PollFailed`.
...
- Represent it as `CacheTime`, a `std::chrono::time_point` over a `CacheClock` tag that deliberately provides no `now()`.
  Clock acquisition belongs to each concrete Backend and must match its Data Plane source; current eBPF and DPDK both
  select elapsed BOOTTIME semantics through different APIs. Each Backend constructs `CacheTime` at one visible place,
  and mixing in a `steady_clock::time_point` stays a compile error.
...
- Allocate 64-bit generations monotonically under the Store mutex. Treat a full counter cycle as outside the physical
  operating envelope rather than adding a theoretical wrap branch to the runtime path.
...
The strict 8E-11 deletion was applied in the same cutover. The legacy C cache/parser/hash/bridge implementation, its C ABI headers, c-ares dependency, ECS build selector, `ebpf.arena_pages`, legacy-only tests, and old DNS microbenchmark were removed. ECS-bearing and other Additional-Section Queries remain explicit Pass-through coverage rather than a dormant ECS-aware implementation. Module 8 implementation work is complete; canonical `PERF-M8-1` evidence is tracked as a deferred TODO while Module 9 is active.
...
`meson compile -C build tidy` was executed and remains nonzero because the current translation-unit set reports include-cleaner findings, existing Config/ProcessControl findings, and one generated-skeleton analyzer warning. No correctness test is skipped because of that result. The frozen `PERF-M8-1` harness and privileged smoke are complete. Canonical performance evidence remains a deferred TODO for final Module 8 closure; it does not block Module 9. No product pass line is introduced by 8E.
