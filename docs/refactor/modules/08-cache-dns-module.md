...
| 8D-11 | `EbpfNativeStorageBinding` boundary and lifetime | Resolved: retain `EbpfNativeSession` as sole owner of skeleton, maps, arena mapping, links, and rings. `prepare_skeleton(layout, secret)` returns one move-only, non-owning binding containing only the cache-map fd and writable arena span after load succeeds; it exposes no skeleton or layout authority. The eBPF Cache module computes one `EbpfCacheStorageLayout` value from validated Cache Config and page size, and Backend passes that same value to Session and Store. A checked adapter writes only the XDP-required fixed-width subset into a shared C `.rodata` ABI with static size/alignment/offset assertions. Store uses a private three-operation `EbpfCacheMap` interface with production fd-backed and deterministic in-memory test adapters; these calls occur only on Cache Fill and cleanup, never the XDP Cache Hit Path. Store does not duplicate fds or remap arena memory. Backend member order plus explicit partial-start/shutdown reset destroys Store before Session release. |
...
| 8D-I3 | Store construction allocation failure | Resolved: a private-constructor `EbpfCacheStore::create()` performs startup allocations, catches only `std::bad_alloc`, and returns `not_enough_memory` through `expected`; Backend maps it to `StartFailed`. Runtime Store operations add no catch blocks. |
...
- Do not add a process-global memory pool. Store startup uses one centralized `bad_alloc` adapter, while Cache Hit, synchronous Cache Fill, and cleanup remain allocation-free after successful startup. Future asynchronous Fill and DPDK packet memory require purpose-specific bounded pools rather than this module owning a general allocator.
...
Decision queue (unresolved, in dependency order):

| # | Decision point | Why it must be resolved before implementation |
|---:|---|---|
| 8E-1 | XDP Query Eligibility parser and shared-vector consumer | Define the exact bounded Query parse/hash sequence, whether Question identity is materialized once for both cache lookup and Pending creation, and how the 19 generated vectors prove lookup/Pending side effects rather than only `XDP_PASS`. |
| 8E-2 | Pending Query physical key and value | Fix the tuple direction, Transaction ID placement, Question fingerprint or canonical facts, authoritative Cache Namespace, `last_seen`, consume state, padding, byte order, and collision assumptions. |
| 8E-3 | Pending map benchmark ownership and selection rule | Reconcile the current 8D-versus-8E timing inconsistency, define equivalent LRU-hash and ordinary-hash implementations, traffic traces, capacity/timeout matrix, cleanup accounting, and the threshold used to select one rather than merely report numbers. |
| 8E-4 | Atomic retransmission refresh and single-consumer correlation | Define multicore synchronization so an identical Query safely refreshes `last_seen`, a mismatch leaves the record live, and only one of concurrent duplicate Responses can consume a complete match and authorize Cache Fill. |
| 8E-5 | Pending consumption versus ring reservation ordering | Decide what happens when correlation succeeds but the packet ring is full or payload copy fails: consume first and lose the fill, reserve first and retain a wider authorization window, or add an explicit claimed state with rollback. |
| 8E-6 | Correlated ring-event ABI and complete-message framing | Define event metadata, byte order, alignment, authoritative Namespace transfer, validated UDP-length rules, maximum reservation size, and variable-length versus fixed-size ring records without truncating responses. |
| 8E-7 | Packet-envelope and topology boundaries | Specify IPv4 fragmentation, IP/UDP length disagreement, checksum handling, VLANs, interface direction, source/destination port rules, and the exact tuple-symmetric behavior when NAT or proxying prevents correlation. |
| 8E-8 | Packet callback and Backend composition ownership | Replace the C bridge with a boundary that lets the Backend-owned `DnsPolicy` and `CacheStore` consume Session-owned ring events synchronously, without moving policy into `EbpfNativeSession` or creating dangling callback context. |
| 8E-9 | Startup, cleanup-worker, and shutdown ordering after cutover | Integrate Store construction, packet ring, Policy, cleanup, partial-start release, and callback quiescence while preserving the existing Operational Loops and making Store destruction precede native-binding release. |
| 8E-10 | Fill-path Bypass/Rejected/error handling before Module 10 | Decide which outcomes remain silent, which use the temporary diagnostic path, and how repeated Store/cleanup errors remain Fail-open without making `poll()` fail or adding observability surfaces early. |
| 8E-11 | Cutover sequence and legacy deletion inventory | Choose whether to switch in one slice or through a short compile-time test seam, prohibit or permit dual-write, and enumerate removal of `loader_cache_bridge`, legacy parser/rebuilder, old key/ECS/admission code, tests, constants, and build targets. |
| 8E-12 | End-to-end acceptance and synchronous-fill performance gate | Define the required correlation, Cache Hit, namespace-isolation, expiry, lifecycle, and DNS-service benchmark results; state what measurement would justify a later bounded asynchronous `FillWork` pool rather than changing the MVP now. |
| 8E-13 | Privileged test and CI execution matrix | Separate unprivileged deterministic tests, verifier/load tests, root network-namespace integration, soak, fuzz smoke, and benchmarks; define which are Module 8 completion gates on the development host and in CI. |
