...
2. `ebpf.arena_pages` is removed from Config Schema. Arena page count is an implementation result derived from `CacheConfig.max_entries`, the resolved runtime slot stride, required arena control/alignment bytes, and the host page size. The eBPF cache map is likewise sized to exactly `max_entries` before skeleton load. This removes two competing descriptions of Cache Capacity: operators configure logical entry capacity and response limit, while the Backend owns their physical page translation. Arithmetic overflow and known static limits are rejected before resource acquisition; failures that depend on kernel map creation or mmap remain Backend `start()` failures.
...
6. Each arena slot persists both `stored_at_ns` and `expires_at_ns`. XDP checks `now >= expires_at_ns` directly and computes per-RR aging from `now - stored_at_ns`; it does not reconstruct one value from a whole-second lifetime on every hit. Store checks timestamp addition and representation overflow before beginning a slot write. The two 64-bit fields also avoid a false space optimization because the slot header is aligned and both expiry and elapsed time are hot-path facts.
...
38. `EbpfCacheStore` has a private constructor and one `create()` factory returning `std::expected<std::unique_ptr<EbpfCacheStore>, std::error_code>`. The factory centralizes all startup allocations and catches only `std::bad_alloc`, mapping it to `std::errc::not_enough_memory`; Backend maps that result to `StartFailed`. Runtime `store()` and `cleanup()` remain `noexcept` and contain no exception guards. Allowing allocation exceptions to escape lifecycle hooks and replacing standard containers with several manually checked nothrow arrays are rejected.
...
41. The MVP introduces no process-global or Backend-wide memory pool. CLI/Config parsing, standard-library objects, libbpf, kernel maps/mappings, and thread stacks have different allocators and lifetimes, while the intended Cache Hit, synchronous Cache Fill, and cleanup paths can already be allocation-free after startup. `EbpfCacheStore::create()` remains the single `std::bad_alloc`-to-`expected` adapter for Store-owned startup allocations, and its inline rollback scratch removes one allocation entirely. A future asynchronous Cache Fill may add its own bounded `FillWorkPool` only if `PERF-8E-2` justifies that feature; DPDK packet memory remains the DPDK Backend's native pool concern.
