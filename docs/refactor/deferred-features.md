...
| DF-13 | DPDK runtime Link State monitoring | Module 9 records one nonblocking Link State snapshot per port during startup and performs no runtime polling, LSC callback, monitor component, or monitor thread. | Reopen after the DPDK packet and Cache Paths are stable. Specify polling versus PMD LSC capability fallback, query-failure semantics, logging rate limits, and stop-before-EAL-cleanup ordering together. | Module 9 decision 28; ADR 0035 |
| DF-14 | DPDK multi-lcore execution | Module 9 separates Client Packet Path, Service Packet Path, Cache Cleanup, and Pending Cleanup as bounded tasks but runs all four cooperatively on the sole MAIN lcore. Task separation is not a concurrency promise. | Obtain a complete single-lcore baseline and suitable physical-PMD evidence that execution is a material bottleneck. Redesign TX queue ownership, mbuf handoff/backpressure, Cache publication/reclamation, Pending concurrency, worker failure, CPU/NUMA placement, and stop/join ordering together. | Module 9 decision 40; ADR 0045 |
