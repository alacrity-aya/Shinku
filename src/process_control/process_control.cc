...
std::expected<void, ProcessControlError> ProcessControl::install_signal_handlers() {
    ...
    auto sigint = ProcessControl::install_handler(SIGINT, "SIGINT");
    if (!sigint)
        return std::unexpected(sigint.error());

    auto sigterm = ProcessControl::install_handler(SIGTERM, "SIGTERM");
    if (!sigterm)
...
void ProcessControl::handle_shutdown_signal([[maybe_unused]] int signal_number) noexcept {
    shutdown_requested_flag = 1;
}

std::expected<void, ProcessControlError>
ProcessControl::install_handler(int signal_number, std::string_view signal_name) {
    struct sigaction action {};
    action.sa_handler = ProcessControl::handle_shutdown_signal;
    sigemptyset(&action.sa_mask);

    if (sigaction(signal_number, &action, nullptr) != 0) {
        return ProcessControl::signal_install_error(signal_name, std::error_code(errno, std::generic_category()));
    }

    return {};
}

std::unexpected<ProcessControlError>
ProcessControl::signal_install_error(std::string_view signal_name, std::error_code error) {
    std::string message("sigaction(");
